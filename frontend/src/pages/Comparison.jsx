    ...
    <div className="animate-fadeIn">
      ...
      {canCompare && (
        <div className="mt-6 space-y-6">
          ...
          {showResults && comparisonData && comparisonData.excluded_tickers?.length > 0 && (
            <div className="card bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
              <div className="flex items-start">
                <svg className="w-5 h-5 text-yellow-600 dark:text-yellow-400 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-300">
                    선택한 기간에 데이터가 없어 일부 종목이 비교에서 제외됐습니다
                  </h3>
                  <p className="mt-1 text-sm text-yellow-700 dark:text-yellow-400">
                    {comparisonData.excluded_tickers
                      .map((ticker) => tickerInfoMap[ticker]?.name || ticker)
                      .join(', ')}{' '}
                    — 추적을 시작한 지 얼마 안 된 종목일 수 있습니다. 날짜 범위를 좁혀보세요.
                  </p>
                </div>
              </div>
            </div>
